#!/usr/bin/env python
"""
This module provides a function to plot the Levelized Cost of Electricity (LCOE) for a PV system.
"""
...
def plot_lcoe(df: pd.DataFrame) -> None:
    ...
    plt.title(f"PV LCOE projection ({df['year'].min()}–{df['year'].max()})", fontsize=18, pad=20)
